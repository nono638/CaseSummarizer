...
**Current Branch:** `main`
**Status:** 🟢 **VOCABULARY EXTRACTION OPTIMIZED | SESSION 14** - Session 14 (2025-11-28) resolved vocabulary extraction hanging on large documents (260 pages / 787KB text). Implemented chunked NLP processing, fixed initialization bugs, and optimized spaCy pipeline. All 55 tests passing. Application runs successfully on large PDFs.

**Latest Session (2025-11-28 Session 14 - Vocabulary Extraction Performance Optimization):**
Fixed vocabulary extraction hanging on large documents. Root causes identified and resolved: (1) **Initialization order bug** - `rarity_threshold` attribute was accessed in `_load_frequency_dataset()` before being set in `__init__`, causing silent failure. (2) **NLTK download timeout** - Added 15-second threading-based timeout to prevent indefinite hangs on network issues. (3) **Large text processing** - Implemented chunked processing with 50KB paragraph-boundary chunks using `nlp.pipe()` for batch processing. (4) **Disabled unused spaCy components** - Removed tagger, lemmatizer, attribute_ruler for ~3x speedup. (5) **Fixed filtering logic** - Medical terms from curated list now always accepted (was incorrectly filtering "cardiomyopathy"), NER-tagged entities (PERSON, ORG, etc.) now trusted regardless of word frequency (was incorrectly filtering "Smith"). Added `VOCABULARY_MAX_TEXT_KB = 200` config constant. All 55 tests passing.
